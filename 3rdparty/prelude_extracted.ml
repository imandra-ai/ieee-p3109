(* extracted by imandrax-extract from "prelude.iml" *)
    ...
    module Int :
      sig
        ...
        val of_int : int -> t
                   ...
                   include Stdlib
                   let of_int i = i end 
    module List = struct type 'a t = 'a list =
                           | [] 
                           | (::) of 'a * 'a t  end
    module String =
      struct
...
[@@builtin.logic_core "mk_s_int"]
type nonrec bool = bool[@@ocaml.doc " Builtin boolean type. "][@@builtin.logic_core
                                                                "mk_s_bool"]
(* skip *)
(* skip *)
...
type nonrec 'a option = 'a option =
  | None 
  | Some of 'a [@@builtin.special "ty.option"][@@noalias ]
type 'a list = 'a Program_prelude_.List.t =
  | [] 
  | (::) of 'a * 'a list [@@builtin.special "ty.list"][@@noalias ]
...
module Map =
  struct
    type (+'a, 'b) t = {
      l: ('a * 'b) list ;
      default: 'b }[@@builtin.logic_core "mk_s_array"]
...
module Set =
  struct
    ...
    let empty_ = Map.const false
    let empty : 'a t = empty_[@@builtin.logic_core "mk_set_empty"]
    let is_empty s : bool= s = empty
    let is_valid s = not (Map.get_default s)[@@macro ]
    let mem : 'a -> 'a t -> bool = Map.get[@@builtin.logic_core
                                            "mk_set_member"]
    let subset_ s1 s2 =
      Map.for_all s1
        ~default:(fun v1 -> if v1 then Map.get_default s2 else true)
        ~f:(fun x v1 -> if v1 then mem x s2 else true)[@@program ]
    ...
    let inter_ (s1 : 'a t) (s2 : 'a t) : 'a t=
      Map.merge s1 s2
        ~default:(fun v1 v2 -> assert (not v1); assert (not v2); false)
        ~f1:(fun _ _ -> None) ~f2:(fun _ _ -> None)
        ~f_both:(fun _ s1 s2 -> if s1 && s2 then Some true else None)
      [@@program ]
    ...
    let union_ (s1 : 'a t) (s2 : 'a t) : 'a t=
      Map.merge
        ~default:(fun v1 v2 -> assert (not v1); assert (not v2); false)
        ~f1:(fun _ _ -> Some true) ~f2:(fun _ _ -> Some true)
        ~f_both:(fun _ s1 s2 -> if s1 || s2 then Some true else None) s1 s2
      [@@program ]
    let union : 'a t -> 'a t -> 'a t = union_[@@builtin.logic_core
                                               "mk_set_union"]
    let complement_ (_s : 'a t) : 'a t= failwith "complement"[@@program ]
    let complement : 'a t -> 'a t = complement_[@@builtin.logic_core
                                                 "mk_set_complement"]
    let diff_ (s1 : 'a t) (s2 : 'a t) : 'a t=
      Map.merge
        ~default:(fun v1 v2 -> assert (not v1); assert (not v2); false)
        ~f1:(fun _ _ -> Some true) ~f2:(fun _ _ -> None)
        ~f_both:(fun _ s1 s2 -> if s1 && (not s2) then Some true else None)
        s1 s2[@@program ]
...
module String =
  struct
    ...
    let unsafe_to_nat : t -> int = Program_prelude_.String.unsafe_to_nat
      [@@builtin.logic_core "mk_unsafe_str_to_nat"][@@builtin.logic_core
                                                     "is_str_to_int"]
...
module Float =
  struct
    ...
    module Round =
      struct
        type t =
          ...
          | Towards_positive 
          | Towards_negative 
          | Towards_zero [@@builtin.logic_core "mk_s_float_rounding"]
    ...
    let is_zero x = Stdlib.(=) x (Stdlib.float_of_string "0.")[@@program ]
    let is_nan (x : t) : bool=
    ...
    let is_positive (x : t) : bool= x > (Stdlib.float_of_string "0.")
      [@@program ]
    let is_negative (x : t) : bool= x < (Stdlib.float_of_string "0.")
      [@@program ]
...
module LString =
  struct
    ...
    let rec length (s : t) =
      match s with
      | [] -> Z.of_nativeint 0n
      | _::tl -> (Z.of_nativeint 1n) + (length tl)[@@adm s]
    let pp out l =
    ...
    let rec for_all f (s : t) =
      match s with | [] -> true | x::tl -> (f x) && (for_all f tl)[@@adm s]
    let rec exists f (s : t) =
      match s with | [] -> false | x::tl -> (f x) || (exists f tl)[@@adm s]
    let rec concat sep =
      function
      | [] -> []
      | s::[] -> s
