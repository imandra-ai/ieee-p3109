...
let neg_pinf = Float.negate f f (SaturationMode.SatNone, RoundingMode.NearestTiesToEven) (Result.get_ok (Float.pinf f));;
...
let chk_neg x f =
  Printf.printf "-(%s) = %s\n" (Float.to_string f x)
    (Float.to_string f (Float.negate f f (SaturationMode.SatNone, RoundingMode.NearestTiesToEven) x))
